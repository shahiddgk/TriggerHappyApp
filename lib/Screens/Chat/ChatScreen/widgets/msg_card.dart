...
class _MessageCardState extends State<MessageCard> {
  ...
  Widget build(BuildContext context) {
    bool isMe="166"==widget.messages.fromId;
    return InkWell(
      onLongPress: () {
      ...
      children: [
        Flexible(
          child: Container(
            ...
            decoration: BoxDecoration(color: AppColors.hoverColor,
      ...
      children: [
        ...
        Flexible(
          child: Container(
            ...
            // to send images
            widget.messages.type==Type.text
            ?Text(widget.messages.msg,style: GoogleFonts.balooBhai2(fontSize: 18),):
            ClipRRect(
      borderRadius: BorderRadius.circular(15),
      child: CachedNetworkImage(
        placeholder: (context,url)=> const Padding(
   ...
   builder: (_){
    return ListView(
      ...
      children: [
        ...
        // copy text 
       widget.messages.type==Type.text? 
       _OptionItem(icon: const Icon(Icons.copy,color: Colors.blue,size: 26,),
       name: 'Copy Text',
        onTap: () async {
      ...
      // save image
       :_OptionItem(icon: const Icon(Icons.download_rounded,color: Colors.blue,size: 26,),
        name: 'Save Image', 
        onTap: () async {
            try {
      ...
      if(widget.messages.type==Type.text && isMe)
        _OptionItem(icon: const Icon(Icons.edit,color: Colors.blue,size: 26,),
         name: 'Edit Message',
          onTap: (){
            Navigator.pop(context);
        ...
        // del msg
      if (isMe)
        _OptionItem(icon: const Icon(Icons.delete,color: Colors.red,size: 26,),
         name: 'Delete Message',
          onTap: () async {
        ...
        // sent time
         _OptionItem(icon: const Icon(Icons.remove_red_eye,color: Colors.blue,size: 26,),
         name: 'Send At:  ${MyDate.getMessageTime(context: context, time: widget.messages.send)}', 
         onTap: (){}),


        // read time
        _OptionItem(icon: const Icon(Icons.remove_red_eye,color: Colors.green,size: 26,),
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            icon,
            Flexible(child: Text('    $name',style: const TextStyle(fontSize: 16,letterSpacing: .5,color: Colors.black87),)),
          ],
        ),
      ),
    );
